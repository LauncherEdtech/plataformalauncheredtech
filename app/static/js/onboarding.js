...
// Sistema de Onboarding - VERSÃO COMPLETA E CORRIGIDA
// ✅ Modal explicativo de simulados + Métricas com scroll livre
// ✅ Todos os ícones de diamante substituídos por Launcher Coins
...
class OnboardingManager {
    ...
    async showCurrentStep() {
        ...
        // ✅ Métricas: Não redirecionar, apenas mostrar modal
        if (this.dadosEtapa.nome === 'metricas') {
            if (currentPath.startsWith('/progresso')) {
                console.log('✅ Já está na página de Métricas');
                this.showMetricas();
                return;
            }
        }
        ...
        switch (this.dadosEtapa.nome) {
            ...
            case 'primeira_aula':
                ...
                break;
            case 'explicacao_simulados':
                this.showExplicacaoSimulados();
                break;
    ...
    // ✅ NOVA ETAPA: Explicação de Simulados
    showExplicacaoSimulados() {
        const progresso = 60;
        const totalEtapas = 5;
            ...
            <div class="onboarding-body">
                ...
                <div class="onboarding-actions">
                    ...
                    <button class="onboarding-btn onboarding-btn-primary" 
                            onclick="onboardingManager.avancarEtapa('entender_simulados')">
                        👍 Entendi, Vamos Lá!
                    </button>
                </div>
            </div>
        ...
        this.modal.innerHTML = `
            ...
            <div class="onboarding-body">
                ...
                <div class="onboarding-progress">
                    ...
                    <div class="progress-text">Etapa 4 de ${totalEtapas}</div>
                </div>
    ...
    // ✅ CORREÇÃO: Métricas agora permite scroll livre por 15 segundos
    showMetricas() {
        console.log('📊 Iniciando etapa de métricas com timer de 15s');
        
        // Minimizar modal automaticamente para permitir navegação
        this.hideModal();
        
        // Toast flutuante no canto superior direito
        const toast = document.createElement('div');
        toast.id = 'onboarding-metricas-toast';
        toast.style.cssText = `
            position: fixed;
            top: 20px;
            right: 20px;
            ...
            border-radius: 12px;
            padding: 20px;
            box-shadow: 0 10px 25px rgba(0,0,0,0.3), 0 0 0 1px rgba(59, 130, 246, 0.2);
            min-width: 300px;
        ...
        toast.innerHTML = `
            <div style="text-align: center;">
                ...
                <div style="color: #ffffff; font-weight: 600; font-size: 1.1rem; margin-bottom: 8px;">
                    Explore suas Métricas!
                </div>
                <div style="color: #9ca3af; font-size: 0.9rem; margin-bottom: 12px;">
                    Role a página e veja suas estatísticas
                </div>
                <div style="background: rgba(59, 130, 246, 0.1); border-radius: 8px; padding: 12px; margin-bottom: 12px;">
                    <div style="color: #60a5fa; font-size: 0.85rem; margin-bottom: 4px;">
                        Avançando automaticamente em:
                    </div>
                    <div id="metricas-countdown" style="color: #3b82f6; font-size: 1.5rem; font-weight: 700;">
                        15s
                    </div>
                </div>
                <button id="btn-pular-metricas" 
                        style="width: 100%; padding: 10px; background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.3s;">
                    ⏭️ Avançar Agora
                </button>
            </div>
        `;
        ...
        let segundos = 15;
        const countdownEl = document.getElementById('metricas-countdown');
        const btnPular = document.getElementById('btn-pular-metricas');
        
        // Hover no botão
        btnPular.addEventListener('mouseenter', () => {
            btnPular.style.transform = 'translateY(-2px)';
            btnPular.style.boxShadow = '0 8px 20px rgba(59, 130, 246, 0.4)';
        });
        
        btnPular.addEventListener('mouseleave', () => {
            btnPular.style.transform = 'translateY(0)';
            btnPular.style.boxShadow = 'none';
        });
        
        // Função para avançar
        const avancar = async () => {
            if (this.metricasTimer) {
                clearInterval(this.metricasTimer);
                this.metricasTimer = null;
            }
            ...
            await this.avancarEtapa('visualizar_metricas');
        };
        ...
        // Timer automático
        this.metricasTimer = setInterval(() => {
            segundos--;
            if (countdownEl) {
                countdownEl.textContent = `${segundos}s`;
            }
    ...
    async pularEtapaAtual() {
        try {
            ...
            const acoesPorEtapa = {
                ...
                'explicacao_simulados': 'entender_simulados',
    ...
    async advanceStep(acao) {
        try {
            ...
            if (data.status === 'ativo') {
                ...
                this.dadosEtapa = data.dados_etapa;
                this._xpTotalGanho = data.xp_total_ganho;
                this._diamantesTotalGanho = data.diamantes_total_ganho;
                this.showCurrentStep();
            }
