...
class RankingUtils:
    ...
    @staticmethod
    def calcular_ranking_geral_otimizado(limit=50):
        """
        Versão otimizada com eager loading de relacionamentos
        """
        from app.models.user import User
        from sqlalchemy.orm import joinedload
        
        return User.query.options(
            joinedload(User.badge_atual)  # Se tiver badge
        ).filter(
            User.is_active == True,
            User.xp_total > 0
        ).order_by(User.xp_total.desc()).limit(limit).all()
    ...
    def get_user_stats_summary(user_id):
        """
        Retorna resumo completo do usuário para o ranking
        """
        from app.models.user import User
        from app.models.estatisticas import XpGanho, TempoEstudo
        from app.models.simulado import Simulado
        ...
        xp_mes = db.session.query(
            func.coalesce(func.sum(XpGanho.quantidade), 0)
        ).filter(
            XpGanho.user_id == user_id,
            XpGanho.data >= inicio_mes
        ).scalar()
        
        tempo_mes = db.session.query(
            func.coalesce(func.sum(TempoEstudo.minutos), 0)
        ).filter(
            TempoEstudo.user_id == user_id,
            TempoEstudo.data_inicio >= inicio_mes
        ).scalar()
        
        simulados_stats = db.session.query(
        ...
        return {
            ...
            'xp_mes': xp_mes,
            'tempo_mes': tempo_mes,
            'tempo_formatado': RankingUtils.formatar_tempo_estudo(tempo_mes),
            'media_simulados': round(simulados_stats.media, 1) if simulados_stats.media else 0,
            'total_simulados': simulados_stats.total if simulados_stats.total else 0
        }
