...
def api_notificacoes():
    """
    Retorna notificações do usuário em JSON
    """
    ...
    for notif in notificacoes:
        # Calcular tempo relativo
        tempo_delta = datetime.utcnow() - notif.data_criacao
        if tempo_delta.days > 0:
            tempo = f"{tempo_delta.days}d"
        elif tempo_delta.seconds >= 3600:
            tempo = f"{tempo_delta.seconds // 3600}h"
        ...
        # Foto do usuário de origem
        foto = None
        origem_nome = "Sistema"
        if notif.origem_user and notif.origem_user.perfil_social:
            apply_profile_media(notif.origem_user.perfil_social)
            foto = resolve_media_url(notif.origem_user.perfil_social.foto_perfil)
            origem_nome = notif.origem_user.nome_completo
        
        # Link da notificação
        link = None
        if notif.post_id:
            link = url_for('helpzone.detalhes_post', post_id=notif.post_id)
        elif notif.tipo == 'follow' and notif.origem_user_id:
            link = url_for('helpzone.perfil_usuario', user_id=notif.origem_user_id)
...
def marcar_notificacao_lida(notif_id):
    ...
    """
    notif = NotificacaoSocial.query.get_or_404(notif_id)
...
def api_ranking_seguidores():
    """
    Retorna ranking de XP entre usuários que você segue
    """
    from app.models.user import User
    
    # IDs de quem você segue
    seguindo_ids = [s.seguido_id for s in current_user.seguindo.all()]
    ...
    for user in usuarios:
        foto = None
        if user.perfil_social:
            apply_profile_media(user.perfil_social)
            foto = resolve_media_url(user.perfil_social.foto_perfil)
