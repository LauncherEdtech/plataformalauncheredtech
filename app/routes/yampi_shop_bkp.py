# app/routes/yampi_shop.py - VERSÃO CORRIGIDA COM PURCHASE_URL
...
def admin_salvar_produtos():
    """Salva produtos capturando TODAS as imagens, DESCRIÇÃO HTML e PURCHASE_URL"""
    if not current_user.is_admin:
    ...
    try:
        ...
        for p in produtos_data:
            try:
                # ===== CORREÇÃO 2: EXTRAIR PURCHASE_URL =====
                # 1. Extração de Preço e PURCHASE_URL
                preco = 97.00
                purchase_url = None  # ← NOVO!
                
                skus = p.get('skus', {}).get('data', []) if isinstance(p.get('skus'), dict) else []
                if skus:
                    prices = skus[0].get('prices', {}).get('data', {})
                    preco = float(prices.get('price_sale', 0) or prices.get('price', 0) or 97.00)
                    
                    # ===== EXTRAIR PURCHASE_URL DO SKU (CRÍTICO!) =====
                    purchase_url = skus[0].get('purchase_url', '')
                
                # 2. Extração AVANÇADA de Imagens
                imagem_capa = ''
                lista_todas_imagens = []
                ...
                # 3. Extração da DESCRIÇÃO (HTML)
                descricao_html = ''
                
                # Tenta pegar de texts.data.description (estrutura V2)
        ...
        db.session.commit()
        return jsonify({'success': True, 'message': f'{sincronizados} produtos sincronizados!'})
